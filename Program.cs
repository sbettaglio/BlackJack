  ...
  class Program
  {
    static void Main(string[] args)
    {
      ...
      while (play)
      {
        ...
        var playerHandSplit = new List<Card>();
        ...
        Console.WriteLine($"Dealer is showing {dealerHand[0].DisplayCard()}, with a showing total of {dealerHand[0].GetCardValue()}");
        if (playerHand[0].GetCardValue() == playerHand[1].GetCardValue())
        {
          Console.WriteLine($"Do you want to split? (YES) or (NO) ");
          var split = Console.ReadLine().ToLower();
          if (split == "yes")
          {
            playerHandSplit.Add(playerHand[1]);
            playerHandSplit.Add(deck[1]);
            playerHand.Add(deck[2]);
            ...
            Console.WriteLine($"Player's total is {playerTotal}");
            Console.WriteLine("Player's split is Holding.");
            for (var i = 0; i < playerHand.Count; i++)
            {
              Console.WriteLine($"{playerHand[i].DisplayCard()}");
            }
            var playerTotalSplit = playerHandSplit[0].GetCardValue() + playerHandSplit[1].GetCardValue();
            Console.WriteLine($"Player's split total is {playerTotalSplit}");
          }
        }
        ...
        else if (playerTotal == 21 && dealerTotal == 21)
        {
          ...
          Console.WriteLine($"dDealer's total is {dealerTotal}");
        }
